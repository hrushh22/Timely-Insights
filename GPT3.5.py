...
openai.api_key = "sk-xyz" # Replace with your actual openai api key.

# Define function to summarize text using GPT-3.5 model
...
# Load JSON data
json_file_path = 'output1_GPT.json'
json_output_path = 'output1_GPT.json'
...
for field, field_data in tqdm(json_data.items(), desc="Processing items"):
    for item_id, item_info in field_data.get("itemInfo", {}).items():
        try:
            ...
            if "gpt_summary" not in item_info:
                if "transcripts" in item_info:
                    ...
                    item_info["gpt_summary"] = final_summary
                    ...
                    with open(json_output_path, 'w') as file:
                        ...
                        print(f"GPT-3.5 summary saved for item {item_id}")
            else:
                print(f"GPT-3.5 summary already exists for {item_id}")
